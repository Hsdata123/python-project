...
def daliy_data_bash(df,live_room=['日期', '周', '月']):
    ...
    daily_df_return = df[use_col].groupby(['日期','是否退款']).agg({
        '主订单编号':'nunique'
    }).reset_index()
    daily_df_return = daily_df_return[daily_df_return["是否退款"]==1]
    daily_df_return = daily_df_return.rename(columns={"主订单编号":"退款订单数"})
    # 计算指标
    daily_df_split = daily_df[['日期','周','月','平台实际承担优惠金额','退款金额','订单应付金额','商家实际承担优惠金额','达人实际承担优惠金额'
                               ,'自播销售金额','达播销售金额','小店自卖销售金额','商品卡销售金额','短视频销售金额','总订单数']].copy()
    daily_df_split['店铺GMV(已加平台券)未扣退'] = daily_df_split['订单应付金额'] + daily_df_split["平台实际承担优惠金额"]
    daily_df_split['扣退店铺成交(已加平台券)'] = daily_df_split['店铺GMV(已加平台券)未扣退'] - daily_df_split['退款金额']
    daily_df_split['扣退店铺成交(未加平台券)'] = daily_df_split['扣退店铺成交(已加平台券)'] - daily_df_split['商家实际承担优惠金额']
    ...
    ##合并退款订单数据
    daily_df_merge = daily_df_split.merge(daily_df_return,on=['日期'])
    ##总单数(已扣退)
...
if __name__ == "__main__":
    ...
    name_list = [{"name":"弹动官方旗舰店","data":"1762333538_3647dc0b62e05b9e030e1167bfd73562cSqUBCXQ.csv","live_room":"鱼子酱"}
                 ,{"name":"弹动个人护理旗舰店","data":"1762333879_618eab2e85953d9593fdb179a56dd2e9NzkMwoHT.csv","live_room":"椰子"}
    ...
    data_daily_self = daily_self_data_bash(data,['日期', '周', '月'])
    ...
    with pd.ExcelWriter('订单数据_弹动.xlsx') as writer:
        ...
        # baiyin_data.to_excel(writer, sheet_name="baiyin_data", index=False)
