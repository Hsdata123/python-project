...
###按照整场汇总千川成交
def qianchuan_group_merge(qianchuan,live_time_day_time):
...
def daliy_data_bash(df,live_room=['日期', '周', '月']):
    ...
    （已扣退）"	商品卡销售占比	短视频销售占比
    """
    df['日期'] = pd.to_datetime(df['支付完成时间']).dt.date
...
if __name__ == "__main__":
    ...
    name_list = [{"name":"弹动官方旗舰店","data":"1763432912_8ec6646a696cd6923e81c9ab71de3685cSqUBCXQ.csv","live_room":"鱼子酱"}
                 ,{"name":"弹动个护旗舰店","data":"1763445194_57d1800a2d7a360bdd29299651c04f67NzkMwoHT.csv","live_room":"椰子"}
    ...
    for name_dict in name_list:
        ...
        if baiyin.shape[0]:
            ...
            quanyu_tran_data = pd.read_excel(latest_file,sheet_name="流量&转化-转化漏斗")[['直播开始时间','日期','直播间曝光-成交转化率(人数)','直播间曝光-观看率(人数)','直播间观看-商品曝光率(人数)','直播间商品曝光-点击率(人数)','直播间商品点击-成交转化率(人数)','直播间观看-成交转化率(人数)','直播间观看人数']]
    ...
    qianchuan_group_merge(qianchuan_data,live_time_day_time_new).to_excel("111.xlsx",index=False)
    live_time_gmv = qianchuan_group_merge(qianchuan_data,live_time_day_time_new)[["直播开始时间","整体成交金额","整体消耗","直播间"]]
    ...
    quanyu_live_data = quanyu_live_data[['直播间','直播开始时间','日期','平均在线人数','直播间曝光-成交转化率(人数)','直播间曝光-观看率(人数)','直播间观看-商品曝光率(人数)','直播间商品曝光-点击率(人数)'
                                         ,'直播间商品点击-成交转化率(人数)','直播间观看-成交转化率(人数)','千次观看成交金额','UV价值']]
    ...
    data["周"] = calculate_week_excel_style_series(data["支付完成时间"])
    data["月"] = data["支付完成时间"].swifter.apply(lambda x:pd.to_datetime(x).month)
    data["是否自播"] = np.where(
