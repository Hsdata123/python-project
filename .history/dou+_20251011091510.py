...
from data_functions import calculate_all_metrics,expand_hours
...
def merge_baiyin_qianchuan_data(end_time,live_room):
    #遍历文件夹下的千川文数据文件
    ...
    print("matched_files:",matched_files,file_list)
...
def live_room_okr_data(merged_data):
    df = pd.DataFrame()
    df = merged_data["基本信息"][["日期","直播结束时间",'直播开始时间',"直播间成交金额","千次观看成交金额","退款金额"]]
    df["直播结束时间"] = df["直播结束时间"].apply(lambda x:x.replace("_"," "))
    df["直播场次计数"] = 1
    # 先将相关列转换为float类型
    df["退款金额"] = df["退款金额"].astype(float)
    df["直播间成交金额"] = df["直播间成交金额"].astype(float)
    ...
    df["观看总时长"] = df.apply(lambda row: 
                    ...
                    axis=1)
    df = df.merge(merged_data["互动&人群&售后"][["直播结束时间",'直播开始时间',"退款订单数","退款人数","新增粉丝数"]],on=["直播结束时间",'直播开始时间'])    

    target_columns = ["平均在线人数","直播间观看人数","直播间曝光人数","商品曝光人数","自然流量观看人数","付费流量观看人数","观看次数",
                            "直播间曝光次数","商品点击人数","成交人数","千川消耗","直播间成交金额","千次观看成交金额",
                            "退款金额","退款订单数","退款人数","直播场次计数","观看总时长","新增粉丝数"]

    df_day = df.groupby(['日期','直播开始时间','直播结束时间','星期'])[target_columns].sum(numeric_only=True).reset_index()
    print(df_day.columns)
    df_day['整体roi'] = df_day.apply(lambda row: 
                        row["直播间成交金额"] / row["千川消耗"] 
                        if row["千川消耗"] != 0 else 0, 
                        axis=1)
    ...
    df_day['客单价'] = df_day.apply(lambda row: 
                    row["直播间成交金额"] / row["成交人数"] 
                    if row["成交人数"] != 0 else 0, 
                    axis=1)  
    # 然后再计算退款率
    df_day['退款率'] = df_day.apply(lambda row: 
                        row["退款金额"] / row["直播间成交金额"] 
                        if row["直播间成交金额"] != 0 else 0, 
                        axis=1)
    df_day['退款后成交金额'] = df_day.apply(lambda row:
                        row["直播间成交金额"] - row["直播间成交金额"]*row['退款率'],
                        axis=1)
    df_day['退款后roi'] = df_day.apply(lambda row: 
                        row["退款后成交金额"] / row["千川消耗"] 
                        if row["千川消耗"] != 0 else 0, 
                        axis=1)
    ...
    df_day["uv价值"] = df_day.apply(lambda row: 
                        row["直播间成交金额"] / row["直播间观看人数"] 
                        if row["直播间观看人数"] != 0 else 0, 
                        axis=1)
...
def main(live_room):
    ...
    baiyin_df_modify,qianchuan_df_modify = merge_baiyin_qianchuan_data(end_time,live_room)
...
def calculate_period_metrics(daily_df, period='W'):
    ...
    # 定义需要求和的列（数值列）
    sum_cols = [
        '直播间观看人数', '直播间成交金额', '千川消耗', '平均在线人数', 
        '成交人数', '成交订单数', '退款金额',
        '退款订单数', '退款人数', '自然流量观看人数', '付费流量观看人数',
        '商品点击人数', '商品曝光人数', '整体消耗', '整体成交金额',
    ...
    # 计算周期比率指标（基于周期汇总数据）
    period_df['整体roi'] = period_df.apply(
        lambda row: row["直播间成交金额"] / row["千川消耗"] if row["千川消耗"] != 0 else 0, 
        axis=1
    )
    ...
    period_df['客单价'] = period_df.apply(
        lambda row: row["直播间成交金额"] / row["成交人数"] if row["成交人数"] != 0 else 0, 
        axis=1
    )
    
    period_df['退款率'] = period_df.apply(
        lambda row: row["退款金额"] / row["直播间成交金额"] if row["直播间成交金额"] != 0 else 0, 
        axis=1
    )
    
    period_df['退款后成交金额'] = period_df.apply(
        lambda row: row["直播间成交金额"] - row["退款金额"], 
        axis=1
    )
    
    period_df['退款后roi'] = period_df.apply(
        lambda row: row["退款后成交金额"] / row["千川消耗"] if row["千川消耗"] != 0 else 0, 
        axis=1
    )
    ...
    period_df['uv价值'] = period_df.apply(
        lambda row: row["直播间成交金额"] / row["直播间观看人数"] if row["直播间观看人数"] != 0 else 0, 
        axis=1
    )
...
if __name__ == "__main__":
    ...
    columns_sort = ['日期','星期','直播间观看人数',"整体消耗",'整体成交金额',"整体支付ROI","观看-成交转化率","点击-成交转化率",
                    ...
                    '退款金额','退款订单数','退款人数','退款率','自然流量观看人数','自然流量观看占比','付费流量观看人数',
