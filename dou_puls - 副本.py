...
def get_qianchuan_from_db():
    """从数据库获取千川数据"""
    connection = pymysql.connect(**QC_DB_CONFIG)
    try:
...
def get_baiyin_from_db():
    """从百应数据库获取所有表数据"""
    connection = pymysql.connect(**BY_DB_CONFIG)
    try:
        # 表名映射
        table_mapping = {
            'basic_info': '基本信息',
            'traffic_conversion_funnel': '流量&转化-转化漏斗',
            'traffic_analysis_channel': '流量分析-渠道分析',
            'traffic_short_video': '流量&转化-短视频引流',
            'interaction_after_sale': '互动&人群&售后',
            'live_room_summary': '直播间总体数据',
            'product_data': '商品数据',
            'sku_data': 'SKU数据',
            'traffic_trend': '流量综合趋势'
        }

        merged_data = {}
        for db_table, sheet_name in table_mapping.items():
            query = f"SELECT * FROM {db_table}"
            df = pd.read_sql(query, connection)
            # 去除列名中的括号
            df.columns = [col.replace('（', '').replace('）', '').replace('(', '').replace(')', '') for col in df.columns]
            merged_data[sheet_name] = df
            print(f"从数据库读取 {db_table} -> {sheet_name}，共 {len(df)} 行")
...
def merge_baiyin_qianchuan_data(live_room):
    ...
    # 从数据库获取百应流量趋势数据
    print("正在从数据库读取百应流量趋势数据...")
    connection = pymysql.connect(**BY_DB_CONFIG)
    try:
        baiyin_df = pd.read_sql("SELECT * FROM traffic_trend", connection)
        print(f"从数据库获取到 {len(baiyin_df)} 条百应数据")
    finally:
        connection.close()

    # 去除列名中的括号
    baiyin_df.columns = [col.replace('（', '').replace('）', '').replace('(', '').replace(')', '') for col in baiyin_df.columns]
...
def live_room_okr_data(merged_data):
    print("=== 调试: merged_data 各表列名 ===")
    for sheet_name, df_debug in merged_data.items():
        if not df_debug.empty:
            print(f"{sheet_name}: {df_debug.columns.tolist()}")

    df = pd.DataFrame()
    df = merged_data["基本信息"][["日期","直播结束时间",'直播开始时间','千次观看成交金额']]
    print(f"基本信息 selected columns: {df.columns.tolist()}, shape: {df.shape}")

    df["直播结束时间"] = df["直播结束时间"].apply(lambda x:x.replace("_"," "))
    df["直播场次计数"] = 1

    df = day_of_week_chinese(df)
    print(f"after day_of_week_chinese: {df.columns.tolist()}, shape: {df.shape}")

    df = df.merge(merged_data["流量分析-渠道分析"][merged_data["流量分析-渠道分析"]["渠道名称"]=="整体"][["直播结束时间",'直播开始时间',"千川消耗","人均观看时长","观看次数"]],on=["直播结束时间",'直播开始时间'])

    # 调试：检查 merge 前后的情况
    print("\n=== 调试: merge 前检查 ===")
    print(f"df (基本信息处理后) 直播结束时间 sample:\n{df['直播结束时间'].head().tolist()}")
    print(f"df (基本信息处理后) 直播开始时间 sample:\n{df['直播开始时间'].head().tolist()}")

    traffic_channel = merged_data["流量分析-渠道分析"][merged_data["流量分析-渠道分析"]["渠道名称"]=="整体"]
    print(f"\n流量分析-渠道分析 直播结束时间 sample:\n{traffic_channel['直播结束时间'].head().tolist()}")
    print(f"流量分析-渠道分析 直播开始时间 sample:\n{traffic_channel['直播开始时间'].head().tolist()}")

    print(f"\n检查两边直播结束时间是否有交集:")
    df_keys = set(df['直播结束时间'].unique())
    tc_keys = set(traffic_channel['直播结束时间'].unique())
    common = df_keys.intersection(tc_keys)
    print(f"基本信息直播结束时间 unique数: {len(df_keys)}, 流量分析直播结束时间 unique数: {len(tc_keys)}, 交集数: {len(common)}")
    if len(common) == 0:
        print(f"示例 - df: {list(df_keys)[:3]}, tc: {list(tc_keys)[:3]}")

    print(f"after merge 流量分析-渠道分析: {df.columns.tolist()}, shape: {df.shape}")
    ...
    print(f"after merge 流量&转化-转化漏斗: {df.columns.tolist()}, shape: {df.shape}")
    ...
    print(f"after merge 互动&人群&售后: {df.columns.tolist()}, shape: {df.shape}")
    ...
    # 确保列名是简单的字符串列表，而不是多级索引
    if isinstance(df_day.columns, pd.MultiIndex):
        df_day.columns = ['_'.join(map(str, col)).strip('_') if isinstance(col, tuple) else col for col in df_day.columns]
    print("df_day.columns",df_day.columns)
    print("df_day dtypes:\n", df_day.dtypes)
    print("df_day shape:", df_day.shape)

    # 计算观看-成交转化率
    if '成交人数' in df_day.columns and '直播间观看人数' in df_day.columns:
        df_day['观看-成交转化率'] = df_day.apply(lambda row:
                        row["成交人数"] / row["直播间观看人数"]
                        if row["直播间观看人数"] != 0 else 0,
                        axis=1)
    else:
        print("警告: 缺少成交人数或直播间观看人数列")
        df_day['观看-成交转化率'] = 0

    if '成交人数' in df_day.columns and '商品点击人数' in df_day.columns:
        df_day['点击-成交转化率'] = df_day.apply(lambda row:
                        row["成交人数"] / row["商品点击人数"]
                        if row["商品点击人数"] != 0 else 0,
                        axis=1)
    else:
        print("警告: 缺少成交人数或商品点击人数列")
        df_day['点击-成交转化率'] = 0

    if '成交人数' in df_day.columns and '直播间曝光人数' in df_day.columns:
    ...
    if '直播间观看人数' in df_day.columns and '直播间曝光人数' in df_day.columns:
...
def main(live_room):
    # 从数据库获取百应数据
    print("开始从数据库读取百应数据...")
    merged_data = get_baiyin_from_db()
